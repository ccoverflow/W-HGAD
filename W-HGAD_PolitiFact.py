...
data2 = sio.loadmat(f'data/politifact.mat')  # help: data/CoAID.mat; data/politifact.mat; data/gossipcop.mat
...
for i in lr:
    ...
    def test():
        ...
        for i in range(data1['news'].x.shape[0]):
            ano_score.append(
                0.47 * torch.norm(out[0][('news', 'to', 'source')][i] - A[('news', 'to', 'source')][i]) + \
                0.47 * torch.norm(out[1]['news'][i] - X['news'][i]) + \
                0.06 * torch.norm(out[2]['news'][i] - torch.FloatTensor(np.array(T['news']))))
        for i in range(data1['source'].x.shape[0]):
            ano_score.append(
                0.47 * torch.norm(out[0][('source', 'to', 'news')][i] - A[('source', 'to', 'news')][i]) + \
                0.47 * torch.norm(out[1]['source'][i] - X['source'][i]) + \
                0.06 * torch.norm(out[2]['source'][i] - torch.FloatTensor(np.array(T['source']))))
